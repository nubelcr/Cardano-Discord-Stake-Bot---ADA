module.exports = {
	...
	subtitle: function(data) {
		...
		const info2 = ['Artist Name', 'Artist ID', 'Artist URL', 'Artist Followers Number', 'Artist Genres', 'Artist Popularity', 'Artist Image (H)', 'Artist Image (M)', 'Artist Image (L)', 'Total Artists Results', 'Artist Data'];
	...
	short_description: "Searches Informations on Spotify",
	...
	action: function(cache) {
		...
		const results = parseInt(data.resultNo);
		const results2 = results - 1
	
		var storage = parseInt(data.storage);
		var varName2 = this.evalMessage(data.varName2, cache);
		...
		if(searchtype === 0) {
			...
			spotifyApi.searchTracks(value, { limit : results})
				.then(function(data) {
					switch(info1) {
						case 0:
			...
			spotifyApi.searchArtists(value, { limit : results})
				.then(function(data) {
					switch(info2) {
						case 0:
			...
			spotifyApi.searchPlaylists(value, { limit : results})
				.then(function(data) {
					switch(info3) {
						case 0:
			...
			spotifyApi.searchAlbums(value, { limit : results})
				.then(function(data) {
					switch(info4) {
						case 0:
